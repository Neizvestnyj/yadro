...
def upgrade() -> None:  # noqa: D103
    ...
    if (
        not op.get_bind()
        .execute(sa.text("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'users')"))
        .scalar()
    ):
        op.create_table(
            "users",
    ...
    bind = op.get_bind()
    indexes = bind.execute(sa.text("SELECT indexname FROM pg_indexes WHERE tablename = 'users'")).fetchall()
